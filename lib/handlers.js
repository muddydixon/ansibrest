...
module.exports = (config, websocket)=>{
  ...
  ansible.checkVersion().then((version)=> config.logger.info(`ansible version ${version}`)).catch(err =>{ throw err; });
  ...
  const execPlaybook = (req ,res ,next)=>{
    ...
    const extraVars = req.query.extraVars;
    debug({
      playbook,
      inventory,
      host,
    ...
    ansible.execPlaybook(playbook, inventory, host, startAt, extraVars, websocket.io).then(({stdout, stderr})=>{
