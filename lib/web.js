...
class Playbook extends Component {
  ...
  render(){
    ...
    return <tr>
      ...
      {inventories.map((inventory, idx)=> <option key={idx} value={inventory}>{inventory}</option>)}
      </select>
      </td>
...
class PlaybookDetail extends Component {
  ...
  render(){
    ...
    //   <h1>{playbook.name}</h1>
    //   <ol>{(playbook.plays || []).map((play, idx)=>{
    //     return <li key={idx}>
    //       <h2>{play.name}</h2>
    //       <ul>{(play.tasks || []).map((task, idy)=>{
    //         return <li key={idy}>{task.name}</li>;
    //       })}</ul>
    //       </li>;
    //   })}</ol>
    //   </div>;
